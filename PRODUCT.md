...
桌面端个人记账工具：在一个工作空间内管理多个账本，覆盖消费记录、分类标签、数据分析、关键事件、日记与股票，让个人财务的记录、回顾与分析都在本地完成。
...
- 功能面：消费记录（模板/分类/标签/账本间同步）、数据分析图表、股票（账户/持仓/交易记录/统计/重置）、关键事件、日记（按账本隔离）、设置（通用/日记/股票/关于）。
