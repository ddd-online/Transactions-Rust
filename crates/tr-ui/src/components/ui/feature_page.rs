...
//!        └─ .page-main
//!             ├─ .page-toolbar    可选：本页操作（底边发丝线；页签下划线压在这条线上）
//!             ├─ .page-content    内容区（吃掉剩余高度）
//!             └─ .page-footer-bar 可选底栏（**只在主列里**，图标条在它左边通到底）
//! ```
//!
//! 三个插槽（`rail` / `toolbar` / `footer`）都是**可选**的：不给就整段不渲染，
//! 不会留下一条空的分隔线。**没有页面级操作就不要给 `toolbar`** —— 空工具栏会白占一条
//! 发丝线，让内容区看上去被削掉一截（股票页的持仓/记录/设置三个子功能就是这样：
//! 它们的主操作属于面板或卡片，不属于整页）。
//!
//! 两条对齐约定：
...
pub fn FeaturePage(
    ...
    #[prop(optional, into)]
    class: Option<String>,
    /// 工具栏（本页操作；**没有页面级操作就别给**，空工具栏只会白占一条发丝线）
    #[prop(optional, into)]
