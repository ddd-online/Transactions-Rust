...
pub enum Icon {
    ...
    /// GitHub（「关于软件」页的外链图标）
    GitHub,
    // ---- 股票页的子功能图标条（Ant Design 官方 outlined 图标）----
    /// 账户（人形）
    User,
    /// 持仓（存款/市值：钞票 + ¥）
    MoneyCollect,
    /// 记录（账本：带书签的账页 + ¥）
    AccountBook,
    /// 参数滑杆（侧栏「股票」；股票域的参数都在设置子功能里，用它比折线更贴切）
    Sliders,
    // ---- 窗口控制 ----
    /// 最小化
    WindowMinimize,
...
impl Icon {
    ...
    pub fn paths(self) -> &'static [&'static str] {
        match self {
            ...
            // 细尖角（Ant Design outlined `right` / `left`）：与 `CaretRight` 的实心三角区分开，
            // 用在"收起/展开侧边栏"这类**位移**语义上 —— 尖角方向就是那一栏会往哪边走。
            Icon::ChevronRight => &[
            ...
            // 瞄准（Ant Design outlined `aim`）：外圈十字准星 + 内圈实心点
            Icon::Aim => &[
                ...
                "M512 392c-32.1 0-62.1 12.4-84.8 35.2-22.7 22.7-35.2 52.7-35.2 84.8s12.5 62.1 35.2 84.8C449.9 619.4 480 632 512 632s62.1-12.5 84.8-35.2C619.4 574.1 632 544 632 512s-12.5-62.1-35.2-84.8A118.57 118.57 0 00512 392z",
            ],
            // 收起（Ant Design outlined `shrink`）：两个对角箭头朝内
            Icon::Shrink => &[
            ...
            // 展开（Ant Design outlined `arrows-alt`）：两个对角箭头朝外（与 `Shrink` 成对）
            Icon::Expand => &[
            ...
            // Ant Design outlined 图标（896 格，原点 64；与全站其余图标同源同风格）
            Icon::User => &[
