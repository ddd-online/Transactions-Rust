# ui-link-event.ps1 —— 「关联到关键事件 / 解除关联」端到端（界面 + 数据库）。
#
# 覆盖点：
...
try {
    ...
    # ================= 2/3 关联到关键事件 =================
    Write-Host "`n[link] 2/3 关联到关键事件：选 $linkDate（当月第 $linkDay 天）"
    $linkButton = Find-RowButton -Window $window -RowText $description -ButtonName '关联到关键事件'
    Assert-True ([bool]$linkButton) '找到「关联到关键事件」按钮'
