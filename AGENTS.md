...
- `ui-smoke`：挨个点开 5 个顶级功能 + 记账页的 4 个子功能（记录 / 分析 / 标签 / 模板，走左侧图标条）
  并断言内容渲染；`-WriteFlow` 还会在工作空间副本里通过界面记一笔（弹窗 → 填表 → 保存 → 列表出现）；
  `-Discover` 导出每页元素清单，用来维护脚本顶部的页面标记表。
- `ui-shots`：抓窗口位图断言每页不是空白，并比较浅/深色的平均亮度；9 张 PNG 落到 `target\ui-shots\`
  供人工验收。补 UIA 的盲区。
