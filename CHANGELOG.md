...
- 「应用设置 → 通用设置」新增 **代理**：`不使用` / **`自动探测`（默认）** / `手动 http://host:port`，
  写在 `~/.transactions.json` 的 `proxy: { mode, url }`（老配置没有这个键时按 `auto` 处理）。
  界面会显示**当前到底走不走代理**（探测到就显示地址与来源），另有「检测」按钮随时复查。
- **自动探测顺序**：环境变量 `ALL_PROXY`/`HTTPS_PROXY`/`HTTP_PROXY` →
  Windows「Internet 选项」的静态代理（`HKCU` → `HKLM`，只读）→ 直连；
  系统里只配了 PAC 自动配置脚本时如实提示"不支持解析"并按直连处理。每次请求重新探测，改系统代理不必重启。
- **只支持 HTTP 代理**：`socks*://` 与 `https://` 明确拒绝并给出中文提示；HTTPS 目标（如 GitHub）
  走同一个 HTTP 代理的 CONNECT 隧道。地址可带 `user:pass@`（Basic 认证）；`host:port` 会自动补 `http://`。
- **真正对所有出网请求生效**：行情查询（`qt.gtimg.cn`）与更新检查/下载（GitHub）读同一份设置，
  不会出现"更新走代理、行情不走"的半生效。
- **行为变更**：以前两处客户端只认进程环境变量里的代理（ureq 的默认行为），Windows 系统代理完全没读；
  现在默认 `auto` 会跟随系统代理，而 `不使用` 是**真正直连**（显式禁用环境变量代理）。
- 回归：`fixtures/ui-proxy.ps1` 用一个本机假 HTTP 代理当判据 —— 手动代理下行情与更新检查
  必须出现在假代理的请求日志里（行情还断言界面显示的是假代理返回的名称），切「不使用」后一条都收不到。

### 命名调整（可见）

- 顶级功能 **「关键事件」更名「事件」**：侧栏导航、页面标题，以及记账页的行内按钮与弹窗标题
  （「关联关键事件」→「关联事件」）和相关提示文案（「查询事件失败」）。
  **只改界面文案**：IPC 命令名（`key_event_*`）、表名（`tbl_billadm_key_event*`）、列名
  （`key_event_date`）、资产目录（`data/assets/key_events`）、路由标识与领域术语都不动。
- 顶级功能 **「股票交易」更名「股票」**：侧栏导航、页面标题、应用设置里的分栏名，以及
  重置相关的可见文案（弹窗标题「重置股票数据」、提示「股票数据已重置」/「重置股票数据失败」）
  一并改名；`fixtures/` 里按名字定位页面的脚本同步更新。
  **只改界面文案**：IPC 命令名、表名、路由标识与领域术语（"股票交易记录"等）都不动。

### 信息架构（可见）

- **顶级功能「数据分析」迁入「记账」，成为第 2 个子功能并更名「分析」**：
  侧栏顶级功能由 6 个减为 **5 个**（记账 / 股票 / 事件 / 日记 / 应用设置），
  图标的排列变成 **记录 / 分析 / 标签 / 模板**（`SubFunction::ALL` 的顺序即图标条顺序）。
  它仍只提供「工具栏 + 内容区」，版心、标题栏与左侧图标条由记账页统一渲染，路由标识 `/da_view` 随之取消。
- `fixtures/` 同步：`ui-smoke` / `ui-shots` / `dev-shot` 的页面表改为 5 个顶级功能 +
  4 个子功能（`分析` 的标志性控件是「新增图表」），`ui-crud` 的图表用例改为走记账页的
  「分析」子功能切页；README / PRODUCT / AGENTS 的功能名与计数一并更新。

### 界面修复

- **悬浮提示（Tip）被压成一列窄条**：「应用设置 → 股票」里过户费/印花税/交易费用说明的气泡
  原本只设了 `max-width`，而绝对定位的包含块是那个十几像素宽的 ⓘ 触发器 —— shrink-to-fit 的可用宽度
  就是它，长文案于是被压成一列窄条并贴着窗口右缘（带出滚动条）。现在气泡先按文案铺开
  （`width: max-content`）再被上限收窄（同时受视口约束），带显式换行的多行说明按原样折行；
  触发器在版心右缘的提示改为右对齐向左铺开（`.ui-tooltip--end`），长气泡不再越出窗口。
- **「应用设置 → 关于软件」的内容不再垂直居中**：页签面板当年从 `.st-pane` 改名为 `.page-pane`
  时漏改了这个面板的类名，于是它退化成普通块级容器（`justify-content` 失效、`margin: auto`
  也垂直居中不了），内容停在顶部。类名补回后居中恢复。
- **侧栏导航的悬浮提示**：`nav_button` 当初把 `title` 写成了 `page.route()` —— `route()` 是
  "页面的稳定标识（仅用于调试）"，不是界面文案，于是鼠标停在侧栏任何一项上，提示都是
  `/accounting_view`、`/da_view` 这类英文路由名。先改成与按钮文字同源的 `page.label()`，
  随后**直接去掉这个 `title`**：侧栏按钮里本来就有可见文字（`.nav-btn-text`），
  再挂一个同名提示只是把旁边的字又说一遍，还多出一块盖住界面的浮层 —— 悬浮提示应当补充
  文字说不清的信息，而不是复述。无障碍名保留 `aria-label`，读屏仍能念出功能名。
- **设置行的最后一行补上分隔线**：`.st-list > .st-card:last-child` 原来有一条
  `border-bottom: none`（"下面就是版心底边"），实测观感是列表被截断、和上面几行不一致 ——
  现在**每一行都有下边线**（通用设置 / 日记配置 / 股票三个分栏一致）。
- **「应用设置 → 关于软件」移除「构建类型：开发版 / 正式版」那一行**：关于页现在只有
  应用名 / 版本 / 更新检查 / GitHub 链接 / 版权行。界面不再消费 `app_info("isDev")`
  （后端能力保留）；`fixtures/ui-about.ps1` 改为**反向断言**这一行不再出现。
- **「关于软件」的 GitHub 链接补上图标**（新增 24 格实心 `Icon::GitHub`，链接改为图标 + 文字一行）。
